...
- **プリフライトの早期打ち切り**: プリフライト（`OPTIONS` + `Access-Control-Request-Method`）は許可・不許可を
  問わず **204 で打ち切る**。許可オリジンのみ `Allow-Methods`/`Allow-Headers`/`Max-Age` と ACAO を付け、不許可
  オリジンには CORS ヘッダを付けない（ブラウザ側で遮断され、ルート情報も漏らさない）。`Vary: Origin` は
  キャッシュ汚染防止のため**常に**付与する。
