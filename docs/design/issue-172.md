...
Cloud Run のコールドスタート・転送時間・コストの観点で、本番イメージを**ランタイムに必要な最小構成**に絞る。ただし #164 で達成した「end-to-end でビルドが通り、コンテナが Prisma Client / bcrypt をロードして起動できる」状態は維持する（リグレッションさせない）。

## 2. スコープ（やること / やらないこと）

### やること
- 本番ステージを「本番依存 + ランタイム成果物のみ」に絞り、devDependencies・ソース・ビルドキャッシュを含めない。
- bcrypt ネイティブ・Prisma クエリエンジン・生成済み Prisma Client がランタイムに存在することを実機起動で確認する。
...
2. 生成イメージのコンテナを起動したとき、`@prisma/client` と `bcrypt` のロードに失敗せず `node server/dist/server.js` が起動する（`Cannot find module` / `Query engine ... not found` / native binding エラーが出ない）。
...
`pnpm deploy` は本番依存を**新規 install** するため、builder で `prisma generate` 済みの Prisma Client（`.pnpm` 仮想ストア内の `@prisma/client` / `.prisma/client`）は `/prod` には自動では入らない。そこで **`/prod` で改めて `prisma generate` を実行**して Prisma Client・クエリエンジンを `/prod` の node_modules に生成する。bcrypt ネイティブは `onlyBuiltDependencies`（#164 で追加済み）により deploy の install 時にビルドされる。

`/prod` には `server/dist`（ビルド成果物）・`server/prisma`（スキーマ・migrations）が含まれる必要がある。`deploy` は対象ワークスペースのファイルを含めるが、`dist` が `.gitignore` 等で除外されないよう、build 後に deploy する順序とし、必要に応じ明示 COPY で補う。

本番ステージは `COPY --from=builder /prod ./server`（自己完結ツリー）に絞り、builder の `/app` 全体は持ち込まない。pnpm 自体も本番ステージには入れない（`prisma generate` は builder 側で完了済み）。
...
- Prisma の生成（`prisma generate`）が本番成果物に対して供給される（builder で generate 済みツリーを使う or 本番向けに generate する）。
...
- `pnpm deploy` の Prisma 生成物取り込みが不確実な場合は、`/prod` で `prisma generate` を再実行して確実化する（本設計で採用）。実機ビルド＆起動で担保する。
