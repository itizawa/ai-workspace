...
ADR-0002 で Turborepo を採用した理由のひとつが「CI 時間短縮」である。その延長として **Turborepo Remote Caching** を CI に導入し、`lint` / `test` / `build` 等のタスクキャッシュを **CI 実行間で共有**する。これにより、入力（ソース / ロックファイル / 環境）に変更のないワークスペースのタスクが `cache hit, replaying logs`（`>>> FULL TURBO`）でスキップされ、典型的な PR（一部 WS のみ変更）の CI リードタイムと Actions 分を削減する。

- **バックエンド: Vercel Remote Cache を採用**。GitHub-hosted ランナー前提でセットアップ負荷が最小（サーバ構築不要）。turbo 公式が標準サポートする。セルフホスト互換（`ducktors/turborepo-remote-cache`）は別途サーバ運用が必要なため不採用。
- **認証情報は GitHub Secrets / Variables 経由**でのみ渡し、リポジトリに平文を残さない。
  - `TURBO_TOKEN`（Vercel アクセストークン・機密） → **GitHub Secrets**。
  - `TURBO_TEAM`（チーム slug・非機密） → **GitHub Variables**（`vars`）。
- **注入箇所**: `.github/workflows/ci.yml` の `build-test-lint` ジョブに `env`（`TURBO_TOKEN` / `TURBO_TEAM`）を定義し、`pnpm turbo run ...` を含む全ステップで共有する。
- **未設定時のフォールバック**: `TURBO_TOKEN` / `TURBO_TEAM` が未設定（空）の環境（ローカル開発・Secrets 未登録の CI）では turbo が Remote を無効化しローカルキャッシュにフォールバックするため、従来どおり緑になる。Remote Caching はあくまで CI の最適化で必須前提ではない。
- **キャッシュキーの健全性**: turbo はタスクハッシュにソース入力・依存タスクのハッシュ・グローバル入力（root の `pnpm-lock.yaml` / `turbo.json` / `package.json` 等）・関連 env を含める。入力が変わったタスクは再実行され、失敗タスクはキャッシュされないため、既存 CI ゲート（lint/test/build の合否判定）の正しさは損なわれない。
- **`turbo.json` の `outputs`**: 各タスクの `outputs`（`build` 系の `dist/**`・`storybook-static/**`、`openapi` の `openapi.json`、`gen-types`・`gen-field-specs` の生成物）は Remote Cache 対象として妥当で不足なし。`lint` / `test` は出力レスで成否のみをキャッシュする（妥当）。`db:generate` / `dev` は `cache: false` で対象外。
- **セットアップ手順**: GitHub リポジトリ Settings → Secrets に `TURBO_TOKEN`、Variables に `TURBO_TEAM` を登録する。同一内容で 2 回 CI を流すと 2 回目のログにキャッシュヒットが観測できる。
...
- 2026-06-14: **Turborepo Remote Caching（Vercel Remote Cache）を CI に導入**（#508。`TURBO_TOKEN` / `TURBO_TEAM` を GitHub Secrets / Variables 経由で `ci.yml` に注入し、変更のないワークスペースの再計算をスキップ。未設定時はローカルキャッシュにフォールバック）。
