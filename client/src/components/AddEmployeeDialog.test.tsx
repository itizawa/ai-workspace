...
// 受け入れ条件 #217: Employee 追加ダイアログの動作
describe("AddEmployeeDialog（#217）", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it("open=false のときダイアログは非表示", () => {
    stubFetch(201, {});
    const onClose = vi.fn();
    renderWithClient(<AddEmployeeDialog open={false} onClose={onClose} />);
  ...
  it("表示名を入力すると追加ボタンが有効になる", async () => {
    ...
    renderWithClient(<AddEmployeeDialog open={true} onClose={onClose} />);
    expect(await screen.findByRole("dialog")).toBeInTheDocument();
    await userEvent.type(screen.getByRole("textbox", { name: "表示名" }), "テスト社員");
    expect(screen.getByRole("button", { name: "追加" })).toBeEnabled();
  });

  it("送信するとEmployee作成APIが呼ばれダイアログが閉じる", async () => {
    const fetchSpy = vi.fn((input: Request | string) => {
      ...
      if (url.includes("/admin/employees")) {
        return Promise.resolve(jsonResponse(201, { id: "new-id", displayName: "テスト社員", isBot: true }));
      }
      return Promise.resolve(jsonResponse(200, []));
    });
