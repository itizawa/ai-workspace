...
describe("runCommunityBatch (#306)", () => {
  ...
  const buildDeps = (
    ...
    // vote 集計（重み算出）用。既定では vote 0（純スコアなし）で全コミュニティが底 +1 のみ。
  ...
  it("rng を固定すると決定的に選ばれたコミュニティのみ生成・永続化される", async () => {
    // vote 0 のため両コミュニティとも底 +1（weight=1）。累積: community1=[0,1), community2=[1,2)。
  ...
  it("vote の純スコアが高いコミュニティほど選ばれやすい（重み付き）", async () => {
    // community2 配下の post (post-c2-*) に多数の up vote を積み、community2 の重みを大きくする。
    const deps = buildDeps([community1, community2]);
    // community2 の post (post-c2-*) に up を 5 件入れる → 純スコア +5、底込み weight=6。
    // community1 は vote 0 → weight=1。total=7。community1=[0,1), community2=[1,7)。
    // この vote を community2 に解決するため、voteRepo.netScoresByCommunitySince を呼ぶので、
  ...
  it("vote 0 の新規コミュニティも底 +1 により選定対象になる", async () => {
  ...
  it("recentLimit を指定すると直近ログ取得件数に反映される（#389 AC2）", async () => {
    const deps = buildDeps([community1]);
    const generate = vi.fn().mockResolvedValue(validGenerationOutput);
    // postRepo.listByCommunity / commentRepo.listByCommunity の limit 引数を観察する。
...
describe("generateSlotKey (#469)", () => {
  ...
  it("UTC とローカル時刻で日付が異なる場合も UTC 日付で返す", () => {
    // UTC 23:30 は JST (UTC+9) では翠日 08:30—日付が変わるケース。
    // UTC 基準なら "2026-06-09T23:30" を返すべき。
    const date = new Date("2026-06-09T23:30:00Z"); // UTC 23:30 = JST 翠日 08:30
