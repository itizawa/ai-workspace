...
describe("selectAppearingMembers (B-2)", () => {
  it("最終登場が古いワーカーを優先して最大 count 名の id を返す", () => {
    ...
    expect(selectAppearingMembers(workers, 2, lastAppearedBySlot)).toEqual(["ken", "mei"]);
  });

  it("未登場（エントリ無し）のワーカーを最優先で選ぶ", () => {
    // ken のみ未登場 → 最優先
    const lastAppearedBySlot = { haru: 2, mei: 4 };
    expect(selectAppearingMembers(workers, 1, lastAppearedBySlot)).toEqual(["ken"]);
  });

  it("候補数が count 以下なら全員返す", () => {
    expect(selectAppearingMembers(workers, 5, {})).toHaveLength(3);
  });

  it("count = 0 なら空配列を返す", () => {
    expect(selectAppearingMembers(workers, 0, { haru: 1 })).toEqual([]);
  });

  it("同入力で結果が決定的（同点は入力順で安定）", () => {
    const slot = { haru: 2, ken: 2, mei: 2 };
    const a = selectAppearingMembers(workers, 2, slot);
    const b = selectAppearingMembers(workers, 2, slot);
    expect(a).toEqual(b);
  ...
  it("入力（workers / lastAppearedBySlot）を破壊しない", () => {
    ...
    selectAppearingMembers(wrkInput, 2, slotInput);
