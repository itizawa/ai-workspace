---
description: Dark Factory ワークフロー向けに GitHub Issue を起票する。ユーザーの要望（$ARGUMENTS）から、concept.md・ADR・既存コードを踏まえて「背景 / 目的 / 受け入れ条件（テストに落とせる粒度）/ 補足」構成の Issue 本文を起こし、df:todo + priority/* ラベル + マイルストーンを付けて gh issue create する。マイルストーンの指示が無ければ聞き返す。実装はしない（起票のみ）。
